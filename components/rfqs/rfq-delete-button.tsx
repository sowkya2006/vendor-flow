...
import { deleteRFQAction } from '@/app/(dashboard)/rfqs/actions'

interface RFQDeleteButtonProps {
  rfqId: string
}

export function RFQDeleteButton({ rfqId }: RFQDeleteButtonProps) {
  ...
  const handleDelete = () => {
    startTransition(async () => {
      try {
        await deleteRFQAction(rfqId)
        // redirect('/rfqs') in the action navigates away before this runs
      } catch (err) {
  ...
  if (confirmOpen) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-[--color-error]/30 bg-[--color-error-bg] px-3 py-1.5">
        ...
        <span className="text-xs text-[--color-error]">Delete this RFQ?</span>
        <Button
          size="sm"
          variant="destructive"
