...
import { deleteWorkflowAction } from '@/app/(dashboard)/approval-workflows/actions'

interface WorkflowDeleteButtonProps {
  workflowId: string
}

export function WorkflowDeleteButton({ workflowId }: WorkflowDeleteButtonProps) {
  ...
  if (confirmOpen) {
    return (
      <div className="flex items-center gap-2 rounded-lg border border-[--color-error]/30 bg-[--color-error-bg] px-3 py-1.5">
        ...
        <span className="text-xs text-[--color-error]">Delete this workflow?</span>
        <Button
          size="sm"
          variant="destructive"
